...
| RAGAS 4 metrics | M4 | `evaluate_ragas()` | Faithfulness đi từ 0.7292 (prompt gốc) → 0.8321 (sau khi siết prompt + temperature=0 + tăng top_k), answer_relevancy tăng mạnh nhất (0.7252→0.8329). Context_recall (0.8583) và context_precision (0.8825) vẫn là 2 metric thấp nhất — chủ yếu do các câu hỏi dạng version-conflict (mật khẩu v1/v2) và multi-hop nhiều bước tính. |
